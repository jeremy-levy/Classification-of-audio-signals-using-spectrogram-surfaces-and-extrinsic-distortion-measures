...
% Alex Naitsat  added few fixes to hangle numerical errors.
function A = laplace_beltrami_my(face,vertex,method)
...
if strcmp(method,'CotFinite')
    %ct1(ct1<0 | isinf(ct1) | isnan(ct1))  =0; %tranculate negative weights 
     ct1(isinf(ct1) | isnan(ct1))  =0; %tranculate negative weights 
end
ew(ind) = ew(ind) + ct1;
ind = eif(:,2)>0;
...
if strcmp(method,'CotFinite')
    %ct2(ct2<0 | isinf(ct2) | isnan(ct2))  =0; %tranculate negative weights 
    ct2(isinf(ct2) | isnan(ct2))  =0; %tranculate negative weights 
end
...
switch method
    ...
    case 'Desbrun'
        ...
        sA = sum(A,2);
        A = A - diag(sA);
    case 'Uniform' %uniform weights , used when cotan weigths caused foldovers 
         A = sparse([edge(:,1);edge(:,2)],[edge(:,2);edge(:,1)],ones(2*ne,1)); 
        sA = sum(A,2);
        A = A - diag(sA);
    otherwise
